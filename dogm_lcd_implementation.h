...
#define encrot3 1
#ifdef DreamMaker
#define LCD_CLICKED ((buttons&EN_C) && (blocking_enc<millis()))
#else
...
//U8GLIB_SSD1309_128X64 u8g(LCD_PINS_ENABLE, LCD_PINS_D7, LCD_PINS_RS);
U8GLIB_NHD27OLED_2X_BW u8g(21, 20, 6, 5, 17);
#else
...
static void lcd_implementation_drawmenu_generic(uint8_t row, const char* pstr, char pre_char, char post_char)
{
    ...
    u8g.setPrintPos((LCD_WIDTH-2)*DOG_CHAR_WIDTH, (row + 1) * DOG_CHAR_HEIGHT);
...
static void lcd_implementation_drawmenu_setting_edit_generic(uint8_t row, const char* pstr, char pre_char, char* data)
{
    ...
    if (*data>=0x80) {
        u8g.setPrintPos((LCD_WIDTH-1-strlen(data)*2)*DOG_CHAR_WIDTH, (row + 1) * DOG_CHAR_HEIGHT);
    }
    else{
        u8g.setPrintPos((LCD_WIDTH-1-strlen(data))*DOG_CHAR_WIDTH, (row + 1) * DOG_CHAR_HEIGHT);
    }
    u8g.print(data);
}
...
static void lcd_implementation_drawmenu_setting_edit_generic_P(uint8_t row, const char* pstr, char pre_char, const char* data)
{
    ...
    u8g.print(':');
	if (pgm_read_byte(data)>=0x80) {
        u8g.setPrintPos((LCD_WIDTH-1-strlen_P(data)*2)*DOG_CHAR_WIDTH, (row + 1) * DOG_CHAR_HEIGHT);
    }
    else{
        u8g.setPrintPos((LCD_WIDTH-1-strlen_P(data))*DOG_CHAR_WIDTH, (row + 1) * DOG_CHAR_HEIGHT);
    }
